...
export const OnHoverMenu = GObject.registerClass({
    ...
    _init(box, settings, gIcon, path, alias, widgetInfo, dataHandler) {
        ...
        this._hoverMode = settings.get_boolean('enable-on-hover-mode');
        this._delayValue = settings.get_int('on-hover-delay');
        settings.connectObject(
            'changed::enable-on-hover-mode', () => {
                this._hoverMode = settings.get_boolean('enable-on-hover-mode');
                this._updateTriggerSignals();
            },
    ...
    _updateTriggerSignals() {
        ...
        this._disconnectClickSignal();

        if (this._hoverMode)
            this._connectHoverSignals();
        else
            this._connectClickSignal();
    }
    ...
    _connectClickSignal() {
        if (!this._buttonPressId)
            this._buttonPressId = this._box.connect('button-press-event', this._onClick.bind(this));
        if (!this._buttonReleaseId)
            this._buttonReleaseId = this._box.connect('button-release-event', this._onClickRelease.bind(this));
    }

    _disconnectClickSignal() {
        if (this._buttonPressId) {
            this._box.disconnect(this._buttonPressId);
            this._buttonPressId = null;
        }
        if (this._buttonReleaseId) {
            this._box.disconnect(this._buttonReleaseId);
            this._buttonReleaseId = null;
        }
    }
